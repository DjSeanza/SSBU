...
class Experiment:
    ...
    def train_and_evaluate_model(self, model_name, X_resampled, y_resampled, datascaler, replication):
        ...
        accuracy, f1, roc_auc, precision, cohen_kappa, predictions = trainer.evaluate(X_test, y_test)

        self.store_results(model_name, replication, accuracy, f1, roc_auc, precision, cohen_kappa, best_params)
    ...
    def store_results(self, model_name, replication, accuracy, f1, roc_auc, precision, cohen_kappa, best_params):
        ...
        logging.info(
            ...
            f"ROC AUC: {roc_auc:.4f}, PRECISION: {precision: .4f} COHEN KAPPA: {cohen_kappa: .4f}  Params: {best_params}")
